...
class HelperDbPCM
{
	...
	{
		return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.$this->definition['table'].'` a
		LEFT JOIN `'._DB_PREFIX_.$this->definition['table'].'_lang` b
		ON b.`'.$this->definition['primary'].'` = a.`'.$this->definition['primary'].'` AND b.`id_lang` = '.(int)Context::getContext()->language->id);
	}

	private function isLang()
	{
		$exists_fields_lang = false;
		foreach ($this->definition['fields'] as $field)
		{
